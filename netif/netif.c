...
{
	//* 临时地址不为空，且匹配则返回临时地址
	if (pstNetif->stIPv6.ubaTmpAddr[0] && !ipv6_addr_cmp(ubaDestination, pstNetif->stIPv6.ubaTmpAddr, pstNetif->stIPv6.ubTAPrefixBitLen))
		return pstNetif->stIPv6.ubaTmpAddr; 

	//* 单播地址不为空，且匹配则返回单播地址
	if (pstNetif->stIPv6.ubaUniAddr[0] && !ipv6_addr_cmp(ubaDestination, pstNetif->stIPv6.ubaUniAddr, pstNetif->stIPv6.ubUAPrefixBitLen))
...
PST_NETIF netif_get_eth_by_ipv6_prefix(UCHAR ubaDestination[16], UCHAR *pubSource, BOOL blIsForSending)
{
	...
	os_thread_mutex_lock(l_hMtxNetif);
	{
		...
		while (pstNextNode)
		{
			...
			{
				//* 临时地址不为空，且匹配则返回临时地址
				if (pstNextNode->stIf.stIPv6.ubaTmpAddr[0] && !ipv6_addr_cmp(ubaDestination, pstNextNode->stIf.stIPv6.ubaTmpAddr, pstNextNode->stIf.stIPv6.ubTAPrefixBitLen))
				{
					pstNetif = &pstNextNode->stIf;
					if (pubSource)
				...
				//* 单播地址不为空，且匹配则返回单播地址
				if (pstNextNode->stIf.stIPv6.ubaUniAddr[0] && !ipv6_addr_cmp(ubaDestination, pstNextNode->stIf.stIPv6.ubaUniAddr, pstNextNode->stIf.stIPv6.ubUAPrefixBitLen))
				{
					pstNetif = &pstNextNode->stIf;
					if (pubSource)
				...
				if (!ipv6_addr_cmp(ubaDestination, pstNextNode->stIf.stIPv6.ubaLnkAddr, pstNextNode->stIf.stIPv6.ubLAPrefixBitLen))
				{
					pstNetif = &pstNextNode->stIf;
					if (pubSource)
