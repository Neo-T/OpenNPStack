...
static INT exit(CHAR argc, CHAR* argv[], ULONGLONG ullNvtHandle); 
#define NVTCMD_BUILTIN_NUM 2 //* NVT自带指令的数量
static const ST_NVTCMD l_staNvtCmd[NVTCMD_BUILTIN_NUM] = {    
    { exit, "exit", "logout and return to the terminal.\r\n" }, 
    { exit, "logout", "same as the <exit> command, logout and return to the terminal.\r\n" }, 
}; 
...
static void nvt_nego_opt_send(PSTCB_NVT pstcbNvt)
{
    ...
    if (ubFilledBytes)
    {
        ...
        send(hRmtTelnetClt, (const char *)ubaSndBuf, ubFilledBytes, 0);
    }
}
...
static void telnet_cmd_send(PSTCB_NVT pstcbNvt, UCHAR ubCmd, UCHAR ubOption)
{
    ...
    send(hRmtTelnetClt, (const char *)&stCmd, sizeof(ST_TELNETPKT_CMD), 0);
}
...
static void telnet_req_term_type(PSTCB_NVT pstcbNvt)
{
    ...
    send(hRmtTelnetClt, (const char *)&stSubOptTermType, sizeof(ST_TELNETPKT_SOPT_TERMTYPE), 0);
}
            ...
            if (pstcbNvt->pszCmdCache[i])
            {
                if (bCmdNum == pstcbNvt->bCmdIdx) //* 匹配
                {
                    ...
                    {                    
                        ...
                        send(hRmtTelnetClt, pszSndBuf, pstcbNvt->bCursorPos + bCpyBytes + 3, 0);
...
static void nvt_char_handler(PSTCB_NVT pstcbNvt, SOCKET hRmtTelnetClt, CHAR ch, BOOL blIsCtlChar)
{
    ...
    if (blIsCtlChar)
    {
        ...
        case 'H': 
            ...
            {                
                memset(pszSndBuf, '\b', pstcbNvt->bCursorPos); 
                send(hRmtTelnetClt, pszSndBuf, pstcbNvt->bCursorPos, 0);
                buddy_free(pszSndBuf);
        ...
        case 'F': 
            ...
            {          
                ...
                send(hRmtTelnetClt, pszSndBuf, i, 0); 
                buddy_free(pszSndBuf); 
            ...
            if (pstcbNvt->bCursorPos < pstcbNvt->bInputBytes)
            {
                ...
                if (3 == pstcbNvt->stSMach.nvt_srv_echo && SMACHNVT_PASSWD != pstcbNvt->stSMach.nvt_state)
                {
                    ...
                    {                                                
                        ...
                        send(hRmtTelnetClt, pszSndBuf, (bCpyBytes << 1) + 1, 0);
                        buddy_free(pszSndBuf); 
                    }
                    else
                        printf("nvt_char_handler() failed, %s\r\n", onps_error(enErr));
                ...
                if (pstcbNvt->bCursorPos > 0)
                {
                    ...
                    if (3 == pstcbNvt->stSMach.nvt_srv_echo && SMACHNVT_PASSWD != pstcbNvt->stSMach.nvt_state)
                    {
                        if (NULL != (pszSndBuf = (CHAR *)buddy_alloc(NVT_ECHO_BUF_LEN_MAX, &enErr)))
                        {
                            ...
                            send(hRmtTelnetClt, pszSndBuf, (bCpyBytes << 1) + 4, 0);
                            buddy_free(pszSndBuf);
                        }
                        else
                            printf("nvt_char_handler() failed, %s\r\n", onps_error(enErr));
...
{    
    ...
    send(hRmtTelnetClt, szFormatBuf, strlen(szFormatBuf), 0);
    send(hRmtTelnetClt, "#\033[01;31mlogin\033[0m: ", sizeof("#\033[01;31mlogin\033[0m: ") - 1, 0);
}
        ...
        if ('\0' == pstcbNvt->szInputCache[i])
        {
            ...
            {                
                ...
                if (strcmp(pszaArg[0], "help"))                
                {
                    ...
                    if (bIsNotFound)
                    {
                        send(hRmtTelnetClt, "Command not supported by terminal: ", sizeof("Command not supported by terminal: ") - 1, 0); 
                        send(hRmtTelnetClt, pszaArg[0], strlen(pszaArg[0]), 0); 
                        send(hRmtTelnetClt, "\r\n", sizeof("\r\n") - 1, 0); 
                    }
                }
                else
...
void thread_nvt_handler(void *pvParam)
{
    ...
    SOCKET hRmtTelnetClt = *(SOCKET *)((UCHAR *)pvParam - offsetof(STCB_TELNETCLT, stcbNvt)); 
    ...
    if (pstcbNvt->pszCmdCache)
    {
        ...
        send(hRmtTelnetClt, "Failed to allocate memory for command cache, you will not be able to browse the command input history by using the up and down arrow keys.\r\n",
    ...
    UINT *punLastOperateTime = (UINT *)((UCHAR *)pvParam - offsetof(STCB_TELNETCLT, stcbNvt) + offsetof(STCB_TELNETCLT, unLastOperateTime));
    CHAR *pbTHIsRunning = (CHAR *)((UCHAR *)pvParam - offsetof(STCB_TELNETCLT, stcbNvt) + offsetof(STCB_TELNETCLT, bTHIsRunning));
    INT nRcvBytes;     
    pstcbNvt->bCursorPos = 0; 
    ...
    while (*pbTHIsRunning)
    {
        ...
        case SMACHNVT_NEGO: 
            ...
            {
                if (os_get_system_secs() - *punLastOperateTime > 3)
                {
                    if (pstcbNvt->stSMach.nvt_try_cnt < 15)
                    {
                        nvt_nego_opt_send(pstcbNvt);
                        *punLastOperateTime = os_get_system_secs();
        ...
        case SMACHNVT_GETTERMNAME: 
            ...
            //* 输出协商配置结果信息
            nvt_print_logo(hRmtTelnetClt); 
            send(hRmtTelnetClt, "=====================================================\r\n", sizeof("=====================================================\r\n") - 1, 0); 
            send(hRmtTelnetClt, "| Welcome to Zion, Your terminal type is ", sizeof("| Welcome to Zion, Your terminal type is ") - 1, 0); 
            ...
            send(hRmtTelnetClt, "| * Character operation mode\r\n| * Remote echo ", sizeof("| * Character operation mode\r\n| * Remote echo ") - 1, 0); 
            ...
            send(hRmtTelnetClt, (const char *)ubaRcvBuf, strlen((const char *)ubaRcvBuf), 0); 
            send(hRmtTelnetClt, "| * Case sensitive mode\r\n=====================================================\r\n", sizeof("| * Case sensitive mode\r\n=====================================================\r\n") - 1, 0);
            
            //* 输出登录信息
            nvt_print_login_info(hRmtTelnetClt); 
        ...
        case SMACHNVT_LOGIN:     
            ...
            if (nRcvBytes)
            {
                if (nRcvBytes > 0)
                {
                    ...
                    //send(hRmtTelnetClt, (const char *)ubaRcvBuf, strlen((const char *)ubaRcvBuf), 0);                    
                    pstcbNvt->stSMach.nvt_try_cnt = 0;
                    nvt_print_passwd_info(hRmtTelnetClt); 
                ...
                else
                {
                    ...
                    if (pstcbNvt->stSMach.nvt_try_cnt < 6)
                    {
                        send(hRmtTelnetClt, "Username is incorrect, please enter again. You have ", sizeof("Username is incorrect, please enter again. You have ") - 1, 0);                          
                        sprintf((CHAR *)ubaRcvBuf, "\033[01;31m%d\033[0m attempts left.\r\n", 6 - pstcbNvt->stSMach.nvt_try_cnt);
                        send(hRmtTelnetClt, (const char *)ubaRcvBuf, strlen((const char *)ubaRcvBuf), 0);
                        nvt_print_login_info(hRmtTelnetClt); //* 用户名错误，再次输出登录信息
                    }
                    else
                    {                       
                        *pbTHIsRunning = FALSE;
        ...
        case SMACHNVT_PASSWD: 
            ...
            if (nRcvBytes)
            {
                ...
                {          
                    ...
                    send(hRmtTelnetClt, (const char *)ubaRcvBuf, strlen((const char *)ubaRcvBuf), 0);
                    send(hRmtTelnetClt, NVT_WELCOME_INFO, sizeof(NVT_WELCOME_INFO) - 1, 0);

                    sprintf((CHAR *)ubaRcvBuf, "* onps stack version: %s\r\n", ONPS_VER);
                    send(hRmtTelnetClt, (const char *)ubaRcvBuf, strlen((const char *)ubaRcvBuf), 0);

                    send(hRmtTelnetClt, "*   Official website:", sizeof("*   Official website:") - 1, 0);
                    sprintf((CHAR *)ubaRcvBuf, " \033[01;34m%s\033[0m\r\n\r\n", ONPS_OFFICIAL_WEB);
                    send(hRmtTelnetClt, (const char *)ubaRcvBuf, strlen((const char *)ubaRcvBuf), 0);

                    nvt_print_ps(hRmtTelnetClt);
                ...
                else
                {
                    ...
                    if (pstcbNvt->stSMach.nvt_try_cnt < 6)
                    {
                        send(hRmtTelnetClt, "Password is incorrect, please enter again. You have ", sizeof("Password is incorrect, please enter again. You have ") - 1, 0);
                        sprintf((CHAR *)ubaRcvBuf, "\033[01;31m%d\033[0m attempts left.\r\n", 6 - pstcbNvt->stSMach.nvt_try_cnt);
                        send(hRmtTelnetClt, (const char *)ubaRcvBuf, strlen((const char *)ubaRcvBuf), 0);
                        nvt_print_passwd_info(hRmtTelnetClt);
                    }
        ...
        case SMACHNVT_INTERACTIVE: 
            telnet_cmd_handler(pstcbNvt, hRmtTelnetClt);
            break; 
        ...
        //* 接收
        nRcvBytes = tcp_recv(hRmtTelnetClt, ubaRcvBuf, sizeof(ubaRcvBuf), 0, 0);
        if (nRcvBytes > 0)
        {
        ...
        else
        {
            ...
            if (nRcvBytes < 0)
            {
                *pbTHIsRunning = FALSE;
                ...
                printf("tcp_recv() failed, the error code: %u\r\n", WSAGetLastError());
                break;
            }
...
static void nego_get_suppress_go_ahead(PSTCB_NVT pstcbNvt, UCHAR ubCmd)
{
    ...
    {
        SOCKET hRmtTelnetClt = *(SOCKET *)((UCHAR *)pstcbNvt - offsetof(STCB_TELNETCLT, stcbNvt));
        send(hRmtTelnetClt, "The client refused the Suppress Go Ahead option on the server and the negotiation was aborted", sizeof("The client refused the Suppress Go Ahead option on the server and the negotiation was aborted") - 1, 0);
    }
...
static INT help(CHAR argc, CHAR* argv[], ULONGLONG ullNvtHandle)
{
    ...
    if (NULL != (pszSndBuf = (CHAR *)buddy_alloc(NVT_INPUT_CACHE_SIZE, &enErr)))
    {
        ...
        while (pstNextNvtCmd)
        {
            ...
            send(hRmtTelnetClt, pszSndBuf, bMemsetBytes + bCmdLen + 1, 0); 
            send(hRmtTelnetClt, pstNextNvtCmd->pstNvtCmd->pszReadme, strlen(pstNextNvtCmd->pstNvtCmd->pszReadme), 0); 
...
{    
    ...
    pstTelnetClt->bTHIsRunning = FALSE; 
    pstTelnetClt->unLastOperateTime = 0;      
