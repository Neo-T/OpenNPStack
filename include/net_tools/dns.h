...
typedef union _UNI_DNS_FLAG_ {
    struct {
        USHORT ReplyCode    : 4; //* 响应码，表示响应的差错状态。0，无差错；1,格式错误；2，服务器自身原因无法处理这个请求；3，域名不存在；4，查询类型不支持；5，服务器拒绝；6~15，保留 
        USHORT resrved      : 3;
        USHORT RecurAvail   : 1; //* 可用递归，响应报文有效。为 1 时，表示服务器支持递归查询，反之则不支持
        USHORT RecurDesired : 1; //* 期望递归，查询报文设置，响应报文返回。为1，告诉名称服务器必须处理这个查询得到结果，这被称作递归查；0，且被请求的名称服务器没有一个授权回答，它将返回一个能解答该查询的其他名称服务器列表，这种方式被称为迭代查询
...
DNS_EXT in_addr_t dns_client_query(INT nClient, const CHAR *pszDomainName, EN_ONPSERR *penErr);
